...
class GenericScrapeJob implements ShouldQueue
{
        ...
        return Cache::remember($cacheKey, now()->addMinutes($cacheTTL), function () {
            ...
            foreach ($this->config['data'] as $keyH => $ospedale) {
                ...
                foreach ($ospedale['data'] as $key => $value) {
                    ...
                    if ($key !== 'extra') {
                        $value['selector'] = $this->resolveSelector($this->config['iterateSelector'] ?? '', $numberIteration, $value['selector']);
                        ...
                        if (isset($value['extra']) && is_array($value['extra'])) {
                            foreach ($value['extra'] as $extra_k => $extra_v) {
                                $extra_v['selector'] = $this->resolveSelector($this->config['iterateSelector'] ?? '', $numberIteration, $extra_v['selector']);
                        ...
                        foreach ($value as $extraK => $extraV) {
                            $extraV['selector'] = $this->resolveSelector($this->config['iterateSelector'] ?? '', $numberIteration, $extraV['selector']);
    ...
    private function resolveSelector(string $iterateSelector, int $numberIteration, string $selector): string
    {
        $offset = $this->config['iterateOffset'] ?? 0;
        if (preg_match('/\\$I([+-]\\d+)?/', $selector, $matches)) {
            $extraOffset = isset($matches[1]) ? (int)$matches[1] : 0;
            $resolvedValue = $numberIteration + $offset + $extraOffset;
            $selector = preg_replace('/\\$I([+-]\\d+)?/', (string)$resolvedValue, $selector, 1);
        }
        return $selector;
    }
